...
qs("#illustrator-location").innerText = store.get(
  "illustrator-presets-dir",
  __("notFound")
);
...
function tryDirectoryAsIllustratorInstallationDirectory(rootDir) {
  ...
  return new Promise((resolve, reject) => {
    fs.readdir(presetDir, (err, localeDirs) => {
      if (err) {
        alert(
          __("cannotFindIllustratorInstallationPleaseSpecifyManually") +
            "\n" +
            err.toString()
        );
        reject();
        ...
        localeDirs = localeDirs
          .map((dir) => path.join(presetDir, dir))
          .filter(isDirectory);
    ...
    if (isWin) {
      ...
      fs.readdir(installDir, (err, directories) => {
        if (err) {
          ...
          const correctDir = directories.find((dir) =>
            dir.startsWith("Adobe Illustrator")
          );
          if (correctDir) {
            const rootDir = path.join(installDir, correctDir);
            tryDirectoryAsIllustratorInstallationDirectory(rootDir)
              .then(resolve)
              .catch(reject);
          } else {
            alert(
...
function createItem(filePath) {
  ...
  fs.lstat(filePath, (err, stat) => {
    if (err) {
      ...
      sizeColumn.innerText = prettyBytes(stat.size);
...
function renderScriptRows() {
  ...
  fs.readdir(scriptsDir, (err, files) => {
    ...
    for (let i = 0; i < scripts.length; i++) {
      ...
      list.appendChild(createItem(path.join(scriptsDir, script)));
    }
  });
}
...
Array.from(qa("[data-tab-controls]")).forEach((tabButton) => {
  tabButton.addEventListener("click", (e) => {
    for (let i = 0; i < tabButton.parentElement.children.length; i++) {
      ...
      if (element.classList.contains("active") && tabButton !== element) {
  ...
  fs.exists(targetDir, (exists) => {
    ...
    const command =
      process.platform === "win32"
        ? `copy "${filepath.replace(/\//g, "\\")}" "${targetDir.replace(
            /\//g,
            "\\"
          )}"`
    ...
    sudo.exec(command, options, function (error, stdout = "", stderr = "") {
      ...
      alert(
        __("successfullyAddedScriptToIllustrator") +
          "\n" +
    ...
    if (!exists) {
      ...
      fs.exists(to, (exists) => {
        if (exists) {
          ...
          const command =
            process.platform === "win32"
              ? `rename "${from.replace(/\//g, "\\")}" "${to.replace(
                  /\//g,
                  "\\"
                )}"`
              : `mv "${from}" "${to}"`;
          ...
          console.log(command);
          sudo.exec(command, options, function (
            error,
            stdout = "",
            stderr = ""
          ) {
            ...
            if (active) {
              ...
              alert(
                __("successfullyDeactivatedScript") +
                  "\n" +
                  __("restartIllustratorToUseTheScript")
              );
            }
    ...
    if (exists) {
      ...
      fs.writeFile(temp, contents, (err) => {
        if (err) {
          ...
          const command =
            process.platform === "win32"
              ? `move "${temp.replace(/\//g, "\\")}" "${scriptsDir.replace(
                  /\//g,
                  "\\"
                )}"`
    ...
    if (await fs.exists(tempFolder)) {
      ...
      request(url, {}, (error, response, body) => {
        ...
        .on("entry", (entry) => {
          ...
          if (
            entry.type === "File" &&
            zipModule.files.some((p) => minimatch(filename, p))
          ) {
...
library.modules.forEach((module, index) => {
  ...
  if (module.description)
    item.appendChild(document.createTextNode(module.description));
  ...
  const renderContent = async () => {
    ...
    while (moduleContent.firstChild) {
        moduleContent.removeChild(moduleContent.lastChild);
    }
    ...
    for (const name of files) {
      ...
      var sizeColumn = document.createElement("div");
      sizeColumn.appendChild(document.createTextNode(prettyBytes(stat.size)));
      sizeColumn.classList.add("column");
      tr.appendChild(sizeColumn);
      list.appendChild(tr);
